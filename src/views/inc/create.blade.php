...
@php echo "@section('title', 'Create new ".ucfirst(strSingular(strtolower($modelName)))."'.)\n"; @endphp
    ...
    <div class="container-fluid">
        ...
        <div class="card">
            ...
            <div class="card-body">
                ...
                @@endforeach
                <form action="{{ $echoStarter }} route('{{ strPlural(strtolower($modelName)) }}.store') }}" method="POST">
                    @@csrf
                    <div class="row">
...
@if ($column_type[$key] == 'json' || $column_type[$key] == 'text' || $column_type[$key] == 'longText')
                        <div class="col-lg-6">
                            <div class="form-group">
                                ...
                                <textarea class="form-control" name="{{ strtolower($col_name) }}" id="{{ strtolower($col_name) }}" placeholder="{{ ucfirst(strtolower($col_name)) }}" rows="5" {{ $required }}>{{ $echoStarter }} old('{{ strtolower($col_name) }}') }}</textarea>
                            </div>
                        </div>
@else
                        <div class="col-lg-6">
                            <div class="form-group">
                                <label for="{{ strtolower($col_name) }}">{!! ucfirst(strtolower($col_name)).' '. $asterisk !!}</label>
                                <input type="text" class="form-control" name="{{ strtolower($col_name) }}" id="{{ strtolower($col_name) }}"  value="{{ $echoStarter }} old('{{ strtolower($col_name) }}') }}" placeholder="{{ strtolower($col_name) }}" {{ $required }}>
                            </div>
                        </div>
@endif
@endif
